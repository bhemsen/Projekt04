...
class Inputhandler:
    ...
    def handleIPv4(self):
        return IPAddress(str(input('Bitte gib eine IPv4 ein:'))).ipv4()

    
    def handleIPv6(self):
        return IPAddress(str(input('Bitte gib eine IPv6 ein:'))).ipv6()
...
# takes Userinput paste it into an Dictionary and returns it
    def handleNewNetwork(self):
        networkadress = str(input('Gib hier die Netzwerkadresse ein: '))
        subnetmask = str(input('Gib hier die Subnetzmaske ein: '))
        hostCount = int(input('Gib hier die Anzahl der benötigten Hostadressen ein: '))

        
        data = {}
        data['networkaddress'] = networkadress
        data['subnetmask'] = subnetmask

        #check how many hosts should be in the subnet and convert it into a Suffix
        data['subnet'] = self.convertHostCountToSuffix(hostCount)

        print(data)
        return data

# connect a IP address to a PC 
    def connectIPtoPC(self, listOfIPs,networkID):
        ...
        for i in range(1,len(listOfIPs)-1):
            ip = str(listOfIPs[i])
            pc = input('PC für '+ip+ ' = ')
            db.addressAssignment(ip, pc, networkID)
